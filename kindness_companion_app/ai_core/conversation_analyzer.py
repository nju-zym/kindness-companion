...
class ConversationAnalyzer:
    ...
    def __init__(self, db_manager):
        ...
        self.max_context_length = 50  # 增加默认上下文长度
        self.max_history_age_days = 30  # 对话历史保留天数
        self.context_switch_threshold = 5  # 上下文切换阈值（分钟）

    def _ensure_tables_exist(self):
        """Ensure that the necessary database tables exist."""
        self.db_manager.connect()

        # Create conversation_history table with additional fields
        ...
        CREATE TABLE IF NOT EXISTS conversation_history (
            ...
            context_id TEXT,
            topic TEXT,  -- 新增：对话主题
            emotion_score FLOAT,  -- 新增：情感分数
            FOREIGN KEY (user_id) REFERENCES users (id)
        )
        """
        )

        # Create conversation_topics table for topic tracking
        self.db_manager.cursor.execute(
            """
    ...
    def get_conversation_history(
        ...
        Args:
            user_id: User ID
            limit: Maximum number of messages to retrieve (defaults to max_context_length)
    ...
    def cleanup_old_conversations(self):
        ...
        """,
            (f"-{self.max_history_age_days} days",),
        )
    ...
    def detect_context_switch(self, user_id: int, new_message: str) -> bool:
        """
        检测是否需要切换对话上下文
        ...
        # 检查最后一条消息的时间
        last_message_time = datetime.fromisoformat(recent_messages[-1]["timestamp"])
        time_diff = datetime.now() - last_message_time

        # 如果时间间隔超过阈值，切换上下文
        if time_diff.total_seconds() > self.context_switch_threshold * 60:
            return True
    ...
    def store_message(
        ...
        Args:
            ...
            topic: Optional topic of the message
            emotion_score: Optional emotion score
        ...
        # 如果没有提供 emotion_score，使用默认值
        if emotion_score is None:
            emotion_score = 0.0

        # 如果没有提供 context_id，创建一个新的
        ...
        message_id = self.db_manager.execute_insert(
            ...
            (user_id, message, is_user, timestamp, context_id, topic, emotion_score)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
            (
                user_id,
    ...
    def _analyze_message_emotion(self, message: str) -> float:
        """分析消息情感分数"""
        # TODO: 实现情感分析逻辑
        return 0.0

    def _extract_message_topic(self, message: str) -> Optional[str]:
        """提取消息主题"""
        # TODO: 实现主题提取逻辑
        return None

    def _update_topic_tracking(self, user_id: int, topic: str):
        """更新主题追踪"""
        self.db_manager.connect()
        ...
        if result:
            ...
            self.db_manager.cursor.execute(
                """
            INSERT INTO conversation_topics (user_id, topic)
            VALUES (?, ?)
            """,
                (user_id, topic),
            )
    ...
    def analyze_user_psychology(self, user_id: int) -> Dict[str, Any]:
        ...
        if not user_messages:
            logger.warning(
                f"No user messages found in conversation history for user {user_id}"
            )
            return {
                "personality_traits": {},
                "communication_preferences": {},
    ...
    def _call_psychology_analysis_api(self, user_messages: List[str]) -> Dict[str, Any]:
        ...
        payload = {
            ...
            "messages": [
                {
                    "role": "system",
                    "content": "你是一个专业的心理分析专家，擅长从文本中分析用户的心理特征。请只返回JSON格式的分析结果。",
                },
    ...
    def _store_psychological_profile(
        ...
        existing_profile = self.db_manager.execute_query(
            "SELECT id FROM user_psychological_profile WHERE user_id = ?", (user_id,)
        )
    ...
    def _update_dialogue_style_preferences(
        self, user_id: int, profile_data: Dict[str, Any]
    ) -> bool:
        """
        ...
        existing_prefs = self.db_manager.execute_query(
            "SELECT id FROM dialogue_style_preferences WHERE user_id = ?", (user_id,)
        )
        ...
        if success:
            ...
            logger.error(
                f"Failed to update dialogue style preferences for user {user_id}"
            )
    ...
    def _map_formality_level(self, comm_prefs: Dict, personality: Dict) -> int:
        ...
        if "formal" in formality_indicators or "professional" in formality_indicators:
            level += 1
        if (
            "very formal" in formality_indicators
            or "highly professional" in formality_indicators
        ):
            level += 1
        if "casual" in formality_indicators or "informal" in formality_indicators:
            level -= 1
        if (
            "very casual" in formality_indicators
            or "very informal" in formality_indicators
        ):
            level -= 1
    ...
    def _map_verbosity_level(self, comm_prefs: Dict, personality: Dict) -> int:
        ...
        if "detailed" in verbosity_indicators or "thorough" in verbosity_indicators:
            level += 1
        if (
            "very detailed" in verbosity_indicators
            or "comprehensive" in verbosity_indicators
        ):
            level += 1
    ...
    def _map_empathy_level(self, comm_prefs: Dict, personality: Dict) -> int:
        ...
        if "empathetic" in empathy_indicators or "supportive" in empathy_indicators:
            level += 1
        if (
            "very empathetic" in empathy_indicators
            or "highly supportive" in empathy_indicators
        ):
            level += 1
        if "factual" in empathy_indicators or "objective" in empathy_indicators:
            level -= 1
        if (
            "very factual" in empathy_indicators
            or "strictly objective" in empathy_indicators
        ):
            level -= 1
    ...
    def get_psychological_profile(self, user_id: int) -> Optional[Dict[str, Any]]:
        ...
        if profile_data:
            try:
                ...
                logger.error(
                    f"Error parsing psychological profile for user {user_id}: {e}"
                )
    ...
    def generate_dialogue_with_style(
        ...
        history = self.get_conversation_history(
            user_id, limit=10, context_id=context_id
        )
    ...
    def _adjust_prompt_for_style(
        self, base_prompt: str, style: Dict[str, int], history: List[Dict]
    ) -> str:
        """
