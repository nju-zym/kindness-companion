...
ZHIPUAI_API_ENDPOINT = (
    "https://open.bigmodel.cn/api/paas/v4/chat/completions"  # Common endpoint
)
...
def generate_pet_dialogue(user_id: int, event_type: str, event_data: dict) -> str:
    ...
    if event_type == "check_in" and "challenge_title" in event_data:
        prompt_parts.append(
            f"They checked in for the challenge: '{event_data['challenge_title']}'. Congratulate them warmly!"
        )
    elif event_type == "reflection_added":
        if "text" in event_data and event_data["text"]:
            prompt_parts.append(
                f"They added a reflection: \"{event_data['text']}\". Respond thoughtfully to their reflection."
            )
            if "analyzed_emotion" in event_data:
                prompt_parts.append(
                    f"The reflection seems to have a '{event_data['analyzed_emotion']}' tone. Adjust your response accordingly (e.g., offer comfort if negative, celebrate if positive)."
                )
            else:
                prompt_parts.append("Acknowledge their effort in reflecting.")
            prompt_parts.append(
                "You could ask a gentle, open-ended question about their experience if appropriate, but don't force it."
            )
        else:
            prompt_parts.append(
                "They added a reflection (content not shown). Acknowledge their effort."
            )
    elif event_type == "app_opened":
        prompt_parts.append(
            "The user just opened the app. Greet them warmly and perhaps offer a gentle encouragement for the day."
        )
    elif event_type == "user_message":
        if "message" in event_data and event_data["message"]:
            prompt_parts.append(
                f"The user sent you a message: \"{event_data['message']}\". Respond directly and engagingly to their message."
            )
            if "analyzed_emotion" in event_data:
                prompt_parts.append(
                    f"Their message seems to have a '{event_data['analyzed_emotion']}' tone. Tailor your response."
                )
        else:
            prompt_parts.append(
                "The user sent you an empty message. Maybe ask if everything is okay?"
            )
...
def _call_dialogue_api(prompt: str) -> str | None:
    ...
    try:
        logger.debug(
            f"Calling ZhipuAI Dialogue API. Endpoint: {ZHIPUAI_API_ENDPOINT}, Model: {DEFAULT_MODEL}"
        )
        ...
        if response_data and "choices" in response_data and response_data["choices"]:
            ...
            if isinstance(message, dict) and "content" in message:
                ...
                if dialogue and dialogue != "[empty]" and dialogue != "[blank]":
                    ...
                    logger.warning(
                        f"ZhipuAI API returned an empty or placeholder dialogue: '{dialogue}'"
                    )
                    return None
            else:
                logger.warning(
                    f"Unexpected response structure from ZhipuAI API: 'message' object is not a dict or missing 'content'. Message: {message}. Response: {response_data}"
                )
                return None
        else:
            logger.warning(
                f"Unexpected response structure or empty choices from ZhipuAI API: {response_data}"
            )
            return None
        ...
        logger.error(
            f"An unexpected error occurred calling ZhipuAI API: {e}", exc_info=True
        )
        return None
