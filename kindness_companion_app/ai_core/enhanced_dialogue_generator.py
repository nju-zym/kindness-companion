...
class EnhancedDialogueGenerator:
    ...
    def generate_dialogue(self, user_id: int, event_type: str, event_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        """
        # Get active context for this user, if any
        context_id = self.active_contexts.get(user_id)

        # For user messages, we want to analyze and store them
        if event_type == 'user_message' and 'message' in event_data:
            ...
            self.conversation_analyzer.store_message(
                user_id, user_message, is_user=True, context_id=context_id
            )

            # Periodically analyze user psychology (e.g., every 10 messages)
            message_count = len(self.conversation_analyzer.get_conversation_history(
                user_id, limit=1000, context_id=None
            ))

            if message_count % 10 == 0:
                logger.info(f"Analyzing psychological profile for user {user_id} after {message_count} messages")
                self.conversation_analyzer.analyze_user_psychology(user_id)

        # Construct base prompt for dialogue generation
        base_prompt = self._construct_base_prompt(user_id, event_type, event_data)

        # Generate dialogue with style adaptation
        dialogue, context_id = self.conversation_analyzer.generate_dialogue_with_style(
        ...
        # If dialogue generation failed, fall back to original generator
        if not dialogue:
            logger.warning(f"Enhanced dialogue generation failed for user {user_id}. Falling back to original generator.")
            dialogue = generate_pet_dialogue(user_id, event_type, event_data)

        # Get current psychological profile for additional context
        ...
        return {
            ...
            'suggested_animation': self._suggest_animation(event_type, event_data, profile)
        }

    def _construct_base_prompt(self, user_id: int, event_type: str, event_data: Dict[str, Any]) -> str:
        """
        ...
        """
        # Start with the core prompt parts
        prompt_parts = [
        ...
        if event_type == 'check_in':
            if 'challenge_title' in event_data:
                prompt_parts.append(f"他们刚刚为挑战\"{event_data['challenge_title']}\"打卡。表示赞赏并鼓励他们继续。")
            else:
            ...
            if 'streak' in event_data:
                ...
                if streak > 1:
                    prompt_parts.append(f"这是他们连续第{streak}天完成这个挑战。表示赞赏他们的坚持。")

        elif event_type == 'reflection_added':
            if 'text' in event_data and event_data['text']:
                prompt_parts.append(f"他们添加了一条反思：\"{event_data['text']}\"。")
                prompt_parts.append("你可以温和地提出一个开放式问题，询问他们的体验，但不要强迫。")
            else:
                prompt_parts.append("他们添加了一条反思（内容未显示）。肯定他们的努力。")

        elif event_type == 'app_opened':
            prompt_parts.append("用户刚刚打开了应用。热情地问候他们，也许可以为这一天提供一些温和的鼓励。")

        elif event_type == 'user_message':
            if 'message' in event_data and event_data['message']:
                prompt_parts.append(f"用户向你发送了一条消息：\"{event_data['message']}\"。直接且有吸引力地回应他们的消息。")
                if 'analyzed_emotion' in event_data:
                    prompt_parts.append(f"他们的消息似乎带有'{event_data['analyzed_emotion']}'的语气。调整你的回应。")
            else:
                prompt_parts.append("用户发送了一条空消息。也许问问他们是否一切都好？")

        # Add final instruction
        prompt_parts.append("现在生成你的回应:")

        # Join all parts into a single prompt
        return "\n".join(prompt_parts)

    def _suggest_animation(self, event_type: str, event_data: Dict[str, Any], profile: Optional[Dict[str, Any]]) -> str:
        """
        ...
        if event_type == 'check_in':
            ...
            if emotion in ['happy', 'content', 'grateful', 'optimistic']:
                ...
                animation = 'excited'
            elif emotion in ['sad', 'anxious', 'worried', 'frustrated', 'angry', 'disappointed', 'stressed', 'negative']:
                animation = 'concerned'
        ...
        if profile:
            ...
            if personality:
                ...
                if personality.get('sensitivity') == 'high' and animation == 'concerned':
        ...
        if animation not in valid_animations:
            logger.warning(f"Invalid animation suggested: {animation}. Falling back to 'idle'.")
            animation = 'idle'
