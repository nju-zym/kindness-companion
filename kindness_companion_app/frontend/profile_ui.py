...
class ProfileWidget(QWidget):
    ...
    def __init__(self, user_manager, progress_tracker, challenge_manager):
        ...
        self.edit_bio_button = QPushButton()
        self.save_bio_button = QPushButton()
        self.cancel_bio_button = QPushButton()
        self.change_avatar_button = QPushButton()
        ...
        self.account_actions_widget.delete_account_requested.connect(
    ...
    def setup_ui(self):
        ...
        self.content_layout.addWidget(
            self.stats_achievements_widget, 3
        )  # Stretch factor 3 (more space)
    ...
    def setup_profile_info_section(self):
        ...
        self.profile_info_frame.setSizePolicy(
        ...
        self.change_avatar_button.setProperty(
            "class", "secondary_button"
        )  # Keep class for QSS
        avatar_layout.addWidget(
            self.change_avatar_button, alignment=Qt.AlignmentFlag.AlignCenter
        )
        ...
        self.bio_display_label.setAlignment(
        ...
        self.bio_display_label.setSizePolicy(
            QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Preferred
        )
        ...
        self.save_bio_button.setProperty(
    ...
    def _set_circular_avatar(self, avatar_data):
        ...
        if not loaded:
            ...
            if pixmap.isNull():
                logger.error(
                    "Default avatar resource ':/icons/user.svg' is also missing!"
                )
                pixmap = QPixmap(90, 90)
                pixmap.fill(QColorConstants.LightGray)
        ...
        final_pixmap.fill(QColorConstants.Transparent)
    ...
    def set_user(self, user):
        ...
        if user:
            ...
            logger.info(
                f"Raw registration_date from user dict: '{reg_date_str_raw}' (Type: {type(reg_date_str_raw)})"
            )
            final_display_text = "未知"
            if reg_date_str_raw:
                try:
                    timestamp_part = (
                        str(reg_date_str_raw)
                        .split(".")[0]
                        .split("+")[0]
                        .replace("T", " ")
                    )
                    ...
                    logger.info(
                        f"Successfully parsed and formatted date: {final_display_text}"
                    )
                except (ValueError, IndexError, TypeError) as e:
                    logger.warning(
                        f"Could not parse registration date '{reg_date_str_raw}': {e}. Falling back to '未知'."
                    )
            else:
                logger.warning(
                    "registration_date key missing or value is None/empty in user dict. Setting text to '未知'."
                )
            ...
            if selected_files:
                ...
                try:
                    ...
                    if temp_pixmap.loadFromData(avatar_bytes):
                        if (
                            temp_pixmap.width() > max_dimension
                            or temp_pixmap.height() > max_dimension
                        ):
                            ...
                            logger.info(
                                f"Resized avatar to fit within {max_dimension}x{max_dimension} ({len(avatar_bytes)} bytes)"
                            )
                        else:
                            logger.info(
                                f"Avatar size is within limits ({len(avatar_bytes)} bytes)"
                            )
                    else:
                        AnimatedMessageBox.showCritical(
                            self, "错误", "无法加载所选图片数据。"
                        )
                        return

                    success = self.user_manager.update_profile(
                        self.current_user["id"], avatar_data=avatar_bytes
                    )
                    ...
                    if success:
                        ...
                        AnimatedMessageBox.showWarning(
                            self, "失败", "无法更新数据库中的头像数据。"
                        )

                except FileNotFoundError:
                    AnimatedMessageBox.showCritical(
                        self, "错误", f"无法找到文件: {source_path}"
                    )
                except Exception as e:
                    logger.error(f"Error changing avatar: {e}")
                    AnimatedMessageBox.showCritical(
                        self, "错误", f"更换头像时发生错误: {e}"
                    )
    ...
    def _handle_delete_account(self):
        ...
        if not ok or not password:
            logger.info(
                f"Account deletion cancelled by user {username} (ID: {user_id}) at password prompt."
            )
            return
