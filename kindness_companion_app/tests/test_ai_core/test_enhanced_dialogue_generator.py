...
from kindness_companion_app.ai_core.enhanced_dialogue_generator import (
...
class TestEnhancedDialogueGenerator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with patch(
            "kindness_companion_app.ai_core.enhanced_dialogue_generator.ConversationAnalyzer"
        ...
        # Sample event data
        self.sample_check_in_event = {"challenge_title": "每日微笑", "streak": 3}
    ...
    def test_generate_dialogue_check_in(self):
        ...
        response = self.generator.generate_dialogue(
            user_id=1, event_type="check_in", event_data=self.sample_check_in_event
        )
        ...
        call_args = (
            self.mock_conversation_analyzer.generate_dialogue_with_style.call_args
        )
        if call_args and len(call_args[0]) > 0:
            ...
            self.assertIn(
                "每日微笑", args[1]
            )  # prompt should contain the challenge title
            self.assertIn("打卡", args[1])  # prompt should mention check-in

        # Check the return value
        self.assertEqual(
            response["dialogue"], self.sample_dialogue_response["dialogue"]
        )
        self.assertEqual(
            response["context_id"], self.sample_dialogue_response["context_id"]
        )
    ...
    def test_generate_dialogue_user_message(self):
        ...
        # Note: store_message is called twice - once for user message and once for AI response
        self.assertEqual(self.mock_conversation_analyzer.store_message.call_count, 2)

        # Check first call (user message)
        args, _ = self.mock_conversation_analyzer.store_message.call_args_list[0]
        self.assertEqual(args[0], 1)  # user_id
    ...
    def test_suggest_animation(self):
        ...
        animation = self.generator._suggest_animation(
            event_type="check_in", event_data={}, profile=None
        )
        ...
        self.assertEqual(animation, "happy")  # positive maps to happy now
